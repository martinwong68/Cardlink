...
export default function OwnerAuditPage() {
  ...
  if (loading) return <div className="flex items-center justify-center py-20"><p className="text-sm text-gray-500">Loading audit log…</p></div>;

  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-xl font-bold text-gray-900">Audit Log</h1>
        <p className="text-xs text-gray-500">{filtered.length} events recorded</p>
      </div>

      <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search actions or resources…" className="w-full rounded-lg border border-gray-100 px-3 py-2 text-sm" />
            ...
            <div key={e.id} className="rounded-xl border border-gray-100 bg-white px-4 py-3">
              <div className="flex items-center justify-between">
                <div className="flex-1">
                  ...
                  <p className="text-[10px] text-gray-500">{e.resource_type}{e.resource_id ? ` · ${e.resource_id.slice(0, 8)}` : ""}</p>
                </div>
                <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${actionBadge(e.action)}`}>{actionLabel(e.action)}</span>
              </div>
              <div className="mt-2 flex items-center justify-between">
                <p className="text-[10px] text-gray-500">IP: {e.ip_address ?? "—"}</p>
                <p className="text-[10px] text-gray-500">{new Date(e.created_at).toLocaleString()}</p>
              </div>
              {e.metadata && Object.keys(e.metadata).length > 0 && (
                <p className="mt-1 text-[10px] text-gray-400">Details: {JSON.stringify(e.metadata).slice(0, 80)}</p>
              )}
            </div>
          ))}
        </div>
