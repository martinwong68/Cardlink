...
export default function OwnerApiKeysPage() {
  ...
  if (loading) return <div className="flex items-center justify-center py-20"><p className="text-sm text-neutral-500">Loading API keys…</p></div>;
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-neutral-500">{keys.length} key{keys.length !== 1 ? "s" : ""} issued</p>
        </div>
      ...
      {keys.length === 0 ? (
        <div className="rounded-xl border border-dashed border-neutral-200 p-8 text-center">
          <p className="text-sm font-medium text-neutral-500">No API keys</p>
          <p className="text-xs text-neutral-400">Create an API key to integrate with external systems.</p>
        </div>
      ) : (
        <div className="space-y-2">
          {keys.map((k) => {
            ...
            return (
              <div key={k.id} className="rounded-xl border border-neutral-100 bg-white px-4 py-3">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-semibold text-neutral-900">{k.name ?? "Unnamed"}</p>
                    <p className="mt-0.5 font-mono text-[10px] text-neutral-500">{masked}</p>
                  </div>
                ...
                <div className="mt-2 flex items-center justify-between">
                  <p className="text-[10px] text-neutral-500">Scopes: {(k.scopes ?? []).join(", ") || "none"}</p>
                  <p className="text-[10px] text-neutral-500">Created {new Date(k.created_at).toLocaleDateString()}</p>
                </div>
                {isActive && (
