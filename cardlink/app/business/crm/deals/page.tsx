...
export default function CrmDealsPage() {
  ...
  if (loading) return <div className="flex items-center justify-center py-20"><p className="text-sm text-neutral-500">Loading deals…</p></div>;

  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-bold text-neutral-900">Deals</h1>
        <div className="flex items-center gap-3">
          <div className="flex overflow-hidden rounded-lg border border-neutral-100">
            <button onClick={() => setViewMode("kanban")} className={`px-3 py-1.5 text-xs font-medium ${viewMode === "kanban" ? "bg-purple-600 text-white" : "text-neutral-600"}`}>Kanban</button>
            <button onClick={() => setViewMode("list")} className={`px-3 py-1.5 text-xs font-medium ${viewMode === "list" ? "bg-purple-600 text-white" : "text-neutral-600"}`}>List</button>
          </div>
      ...
      {showForm && (
        <div className="rounded-xl border border-neutral-100 bg-white p-5 shadow-sm">
          <h2 className="mb-4 text-lg font-bold text-neutral-900">{editId ? "Edit Deal" : "New Deal"}</h2>
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div><label className="mb-1 block text-xs font-medium text-neutral-500">Value</label><input value={value} onChange={(e) => setValue(e.target.value)} type="number" className="w-full rounded-lg border border-neutral-100 px-3 py-2 text-sm" /></div>
              <div><label className="mb-1 block text-xs font-medium text-neutral-500">Probability %</label><input value={probability} onChange={(e) => setProbability(e.target.value)} type="number" max="100" className="w-full rounded-lg border border-neutral-100 px-3 py-2 text-sm" /></div>
            </div>
            <div><label className="mb-1 block text-xs font-medium text-neutral-500">Expected Close Date</label><input value={expectedCloseDate} onChange={(e) => setExpectedCloseDate(e.target.value)} type="date" className="w-full rounded-lg border border-neutral-100 px-3 py-2 text-sm" /></div>
            <div>
              ...
              <div className="flex flex-wrap gap-2">{STAGES.map((s) => (
                <button key={s} onClick={() => setStage(s)} className={`rounded-lg px-3 py-1.5 text-xs font-medium capitalize ${stage === s ? "bg-purple-600 text-white" : "bg-neutral-100 text-neutral-600"}`}>{s}</button>
              ))}</div>
            </div>
      ...
      {viewMode === "kanban" ? (
        <div className="flex gap-3 overflow-x-auto pb-4">
          {STAGES.map((stg) => {
            ...
            return (
              <div key={stg} className={`min-w-[240px] flex-shrink-0 rounded-xl border-t-4 ${STAGE_COLORS[stg] ?? "border-neutral-200"} bg-neutral-50 p-3`}>
                <div className="mb-3 flex items-center justify-between">
                  <span className="text-xs font-bold capitalize text-neutral-700">{stg}</span>
                  <span className="text-xs text-neutral-500">{stageDeals.length} · ${stageValue.toFixed(0)}</span>
                </div>
                <div className="space-y-2">
                  {stageDeals.map((d) => (
                    <button key={d.id} onClick={() => openEdit(d)} className="w-full rounded-lg border border-neutral-100 bg-white p-3 text-left">
                      <p className="text-xs font-semibold text-neutral-900">{d.title}</p>
                      <p className="mt-1 text-base font-bold text-neutral-900">${Number(d.value ?? 0).toFixed(0)}</p>
                      <div className="mt-2 flex items-center justify-between">
                        <span className="text-xs text-neutral-500">{d.probability ?? 0}%</span>
                        {stg !== "won" && stg !== "lost" && (
        ...
        <div className="space-y-4">
          <div className="flex gap-2 overflow-x-auto">
            {["all", ...STAGES].map((s) => (
              <button key={s} onClick={() => setStageFilter(s)} className={`whitespace-nowrap rounded-full px-4 py-1.5 text-xs font-medium capitalize ${stageFilter === s ? "bg-purple-600 text-white" : "bg-neutral-100 text-neutral-600"}`}>{s}</button>
            ))}
          </div>
          {filtered.length === 0 ? (
            <div className="rounded-xl border border-dashed border-neutral-200 p-8 text-center"><p className="text-sm text-neutral-500">No deals found.</p></div>
          ) : (
            <div className="space-y-2">
              {filtered.map((d) => (
                <button key={d.id} onClick={() => openEdit(d)} className="w-full rounded-xl border border-neutral-100 bg-white p-4 text-left transition hover:bg-neutral-50">
                  <div className="flex items-center justify-between">
                    <span className="text-sm font-bold text-neutral-900">{d.title}</span>
                    <span className={`rounded-full px-2 py-0.5 text-xs font-medium capitalize ${BADGE_COLORS[d.stage] ?? "bg-neutral-100 text-neutral-600"}`}>{d.stage}</span>
                  </div>
                  <div className="mt-2 flex items-center justify-between">
                    <span className="text-base font-bold text-neutral-900">${Number(d.value ?? 0).toFixed(0)}</span>
                    <span className="text-xs text-neutral-500">{d.probability ?? 0}% · {d.expected_close_date ?? "No date"}</span>
                  </div>
                </button>
              ))}
            </div>
