...
export default function CrmCampaignsPage() {
  ...
  if (loading) return <div className="flex items-center justify-center py-20"><p className="text-sm text-gray-500">Loading campaigns…</p></div>;
    ...
    <div className="space-y-4">
      ...
      {showForm && (
        <div className="rounded-xl border border-gray-100 bg-white p-5 shadow-sm">
          <h2 className="mb-4 text-lg font-bold text-gray-900">{editId ? "Edit Campaign" : "New Campaign"}</h2>
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div><label className="mb-1 block text-xs font-medium text-gray-500">Start Date</label><input value={startDate} onChange={(e) => setStartDate(e.target.value)} type="date" className="w-full rounded-lg border border-gray-100 px-3 py-2 text-sm" /></div>
              <div><label className="mb-1 block text-xs font-medium text-gray-500">End Date</label><input value={endDate} onChange={(e) => setEndDate(e.target.value)} type="date" className="w-full rounded-lg border border-gray-100 px-3 py-2 text-sm" /></div>
            </div>
            <div><label className="mb-1 block text-xs font-medium text-gray-500">Budget</label><input value={budget} onChange={(e) => setBudget(e.target.value)} type="number" className="w-full rounded-lg border border-gray-100 px-3 py-2 text-sm" /></div>
      ...
      {filtered.length === 0 ? (
        <div className="rounded-xl border border-dashed border-gray-200 p-8 text-center">
          ...
          <p className="text-xs text-gray-400">Create your first marketing campaign.</p>
        </div>
      ) : (
        <div className="space-y-2">
          {filtered.map((c) => (
            <button key={c.id} onClick={() => openEdit(c)} className="w-full rounded-xl border border-gray-100 bg-white p-4 text-left transition hover:bg-gray-50">
              ...
              <p className="text-xs capitalize text-gray-500">{c.type} campaign</p>
              {c.description && <p className="mt-1 text-xs text-gray-500">{c.description}</p>}
              <div className="mt-2 flex items-center justify-between">
                <span className="text-sm font-semibold text-gray-900">Budget: ${Number(c.budget ?? 0).toFixed(0)}</span>
                <div className="flex gap-3">
                  <div className="text-center"><span className="block text-xs font-bold text-gray-900">{c.sent_count ?? 0}</span><span className="text-[10px] text-gray-500">Sent</span></div>
                  <div className="text-center"><span className="block text-xs font-bold text-gray-900">{c.open_count ?? 0}</span><span className="text-[10px] text-gray-500">Opened</span></div>
                  <div className="text-center"><span className="block text-xs font-bold text-gray-900">{c.click_count ?? 0}</span><span className="text-[10px] text-gray-500">Clicked</span></div>
                </div>
              </div>
              <div className="mt-2 flex items-center justify-between">
