...
const statusColors: Record<string, string> = { draft: "bg-neutral-100 text-neutral-600", pending: "bg-yellow-100 text-yellow-700", approved: "bg-green-100 text-green-700", rejected: "bg-red-100 text-red-700", cancelled: "bg-neutral-200 text-neutral-500" };
const priorityColors: Record<string, string> = { low: "bg-blue-100 text-blue-600", normal: "bg-neutral-100 text-neutral-600", high: "bg-orange-100 text-orange-700", urgent: "bg-red-100 text-red-700" };
...
export default function ProcurementRequestsPage() {
  ...
  if (loading) return <div className="flex items-center justify-center py-20"><p className="text-sm text-neutral-500">Loading requests…</p></div>;

  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-neutral-900">Purchase Requests</h1>
          <p className="text-xs text-neutral-500">{requests.length} request(s)</p>
        </div>
      ...
      {showForm && (
        <div className="rounded-xl border border-neutral-100 bg-white p-4 space-y-3">
          <h2 className="text-sm font-bold text-neutral-900">New Purchase Request</h2>
          <input type="text" placeholder="Title *" value={title} onChange={(e) => setTitle(e.target.value)} className="w-full rounded-lg border border-neutral-100 px-3 py-2 text-sm" />
          <textarea placeholder="Description" value={description} onChange={(e) => setDescription(e.target.value)} className="w-full rounded-lg border border-neutral-100 px-3 py-2 text-sm" rows={2} />
          <div className="grid grid-cols-2 gap-3">
            <select value={priority} onChange={(e) => setPriority(e.target.value)} className="rounded-lg border border-neutral-100 px-3 py-2 text-sm">
              <option value="low">Low</option><option value="normal">Normal</option><option value="high">High</option><option value="urgent">Urgent</option>
            </select>
            <input type="number" placeholder="Estimated total" value={estimated} onChange={(e) => setEstimated(e.target.value)} className="rounded-lg border border-neutral-100 px-3 py-2 text-sm" />
          </div>
      ...
      <div className="space-y-2">
        {filtered.length === 0 && <p className="py-8 text-center text-sm text-neutral-400">No purchase requests</p>}
        {filtered.map((r) => (
          <div key={r.id} className="rounded-xl border border-neutral-100 bg-white p-4">
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="text-xs text-neutral-500">{r.pr_number} · Est. ${Number(r.total_estimated).toLocaleString()}</p>
              </div>
              <div className="flex items-center gap-2">
                <span className={`rounded-full px-2 py-0.5 text-[10px] font-semibold ${priorityColors[r.priority] ?? "bg-neutral-100 text-neutral-600"}`}>{r.priority}</span>
                <span className={`rounded-full px-2 py-0.5 text-[10px] font-semibold ${statusColors[r.status] ?? "bg-neutral-100 text-neutral-600"}`}>{r.status}</span>
              </div>
            </div>
            {r.status === "draft" && (
              <div className="mt-2 flex gap-2">
                ...
                <button onClick={() => updateStatus(r.id, "cancelled")} className="rounded-lg bg-neutral-100 px-3 py-1 text-xs text-neutral-500">Cancel</button>
              </div>
            )}
            {r.status === "pending" && (
              <div className="mt-2 flex gap-2">
