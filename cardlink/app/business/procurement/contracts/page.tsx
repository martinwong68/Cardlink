...
const statusColors: Record<string, string> = { draft: "bg-gray-100 text-gray-600", active: "bg-green-100 text-green-700", expired: "bg-yellow-100 text-yellow-700", terminated: "bg-red-100 text-red-700" };
...
export default function ProcurementContractsPage() {
  ...
  if (loading) return <div className="flex items-center justify-center py-20"><p className="text-sm text-gray-500">Loading contracts…</p></div>;
    ...
    <div className="space-y-4">
      ...
      {showForm && (
        <div className="rounded-xl border border-gray-100 bg-white p-4 space-y-3">
          ...
          <input type="text" placeholder="Contract title *" value={title} onChange={(e) => setTitle(e.target.value)} className="w-full rounded-lg border border-gray-100 px-3 py-2 text-sm" />
          ...
          <div className="grid grid-cols-3 gap-3">
            <input type="date" placeholder="Start date" value={startDate} onChange={(e) => setStartDate(e.target.value)} className="rounded-lg border border-gray-100 px-3 py-2 text-sm" />
            <input type="date" placeholder="End date" value={endDate} onChange={(e) => setEndDate(e.target.value)} className="rounded-lg border border-gray-100 px-3 py-2 text-sm" />
            <input type="number" placeholder="Value" value={value} onChange={(e) => setValue(e.target.value)} className="rounded-lg border border-gray-100 px-3 py-2 text-sm" />
          </div>
      ...
      <div className="space-y-2">
        {filtered.length === 0 && <p className="py-8 text-center text-sm text-gray-400">No contracts</p>}
        {filtered.map((c) => (
            ...
            <div className="flex items-start justify-between">
              <div>
                ...
                {(c.start_date || c.end_date) && (
                  <p className="text-xs text-gray-400">{c.start_date ? new Date(c.start_date).toLocaleDateString() : "—"} → {c.end_date ? new Date(c.end_date).toLocaleDateString() : "—"}</p>
                )}
              </div>
              <span className={`rounded-full px-2 py-0.5 text-[10px] font-semibold ${statusColors[c.status] ?? "bg-gray-100 text-gray-600"}`}>{c.status}</span>
            </div>
            {c.status === "draft" && (
              <div className="mt-2 flex gap-2">
                ...
                <button onClick={() => updateStatus(c.id, "terminated")} className="rounded-lg bg-gray-100 px-3 py-1 text-xs text-gray-500">Terminate</button>
              </div>
            )}
            {c.status === "active" && (
