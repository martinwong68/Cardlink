...
export default function PosProductsPage() {
  ...
  if (loading) return <div className="flex items-center justify-center py-20"><p className="text-sm text-gray-500">Loading products…</p></div>;
  ...
  return (
    <div className="space-y-4">
      ...
      <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search by name or SKU…" className="w-full rounded-lg border border-gray-100 px-3 py-2 text-sm" />

      {showForm && (
        <div className="rounded-xl border border-gray-100 bg-white p-5 shadow-sm">
          <h2 className="mb-4 text-lg font-bold text-gray-900">{editId ? "Edit Product" : "New Product"}</h2>
          <div className="space-y-3">
            {[["Name", name, setName], ["SKU", sku, setSku], ["Barcode", barcode, setBarcode], ["Category", category, setCategory]].map(([label, val, setter]) => (
              <div key={label as string}><label className="mb-1 block text-xs font-medium text-gray-500">{label as string}</label><input value={val as string} onChange={(e) => (setter as (v: string) => void)(e.target.value)} className="w-full rounded-lg border border-gray-100 px-3 py-2 text-sm" /></div>
            ))}
            <div className="grid grid-cols-2 gap-3">
              <div><label className="mb-1 block text-xs font-medium text-gray-500">Price</label><input value={price} onChange={(e) => setPrice(e.target.value)} type="number" className="w-full rounded-lg border border-gray-100 px-3 py-2 text-sm" /></div>
              <div><label className="mb-1 block text-xs font-medium text-gray-500">Stock</label><input value={stock} onChange={(e) => setStock(e.target.value)} type="number" className="w-full rounded-lg border border-gray-100 px-3 py-2 text-sm" /></div>
            </div>
      ...
      {filtered.length === 0 ? (
        <div className="rounded-xl border border-dashed border-gray-200 p-8 text-center">
          <p className="text-sm font-medium text-gray-500">No products</p>
          <p className="text-xs text-gray-400">Add your first POS product.</p>
        </div>
      ) : (
        <div className="space-y-2">
          {filtered.map((p) => (
            <button key={p.id} onClick={() => openEdit(p)} className="w-full rounded-xl border border-gray-100 bg-white p-4 text-left transition hover:bg-gray-50">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-bold text-gray-900">{p.name}</p>
                  <p className="text-xs text-gray-500">{p.sku ?? "No SKU"} · {p.category ?? "Uncategorized"}</p>
                </div>
                <div className="text-right">
                  <p className="text-base font-bold text-purple-600">${Number(p.price).toFixed(2)}</p>
                  <p className={`text-xs ${p.stock <= 5 ? "font-semibold text-rose-500" : "text-gray-500"}`}>Stock: {p.stock}</p>
                </div>
              </div>
