...
export default function PosTerminalPage() {
  ...
  if (loading) return <div className="flex items-center justify-center py-20"><p className="text-sm text-neutral-500">Loading terminal…</p></div>;

  return (
    <div className="space-y-4">
      <h1 className="text-xl font-bold text-neutral-900">POS Terminal</h1>
      {noShift && <p className="text-xs text-amber-500">No active shift — orders will not be linked to a shift</p>}

      <div className="grid gap-4 lg:grid-cols-5">
        {/* Product grid */}
        <div className="lg:col-span-3 space-y-3">
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search products…" className="w-full rounded-lg border border-neutral-100 px-3 py-2 text-sm" />
          {filtered.length > 0 ? (
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
              {filtered.map((p) => (
                <button key={p.id} onClick={() => addToCart(p)} className="rounded-xl border border-neutral-100 bg-white p-3 text-left transition hover:bg-neutral-50">
                  <p className="truncate text-sm font-semibold text-neutral-900">{p.name}</p>
                  <p className="text-xs text-neutral-500">{p.sku ?? "No SKU"}</p>
        ...
        {/* Cart */}
        <div className="lg:col-span-2 rounded-xl border border-neutral-100 bg-white p-4">
          <h2 className="mb-3 text-base font-bold text-neutral-900">Cart ({cart.length})</h2>
          {cart.length === 0 ? (
            <p className="py-8 text-center text-xs text-neutral-400">Tap products to add to cart</p>
          ) : (
              ...
              {cart.map((item) => (
                <div key={item.productId} className="flex items-center justify-between py-1">
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-neutral-500">x{item.quantity} @ ${item.price.toFixed(2)}</p>
                  </div>
          ...
          <div className="space-y-1 border-t border-neutral-100 pt-3">
            <div className="flex justify-between"><span className="text-xs text-neutral-500">Subtotal</span><span className="text-xs text-neutral-900">${subtotal.toFixed(2)}</span></div>
            <div className="flex justify-between"><span className="text-xs text-neutral-500">Tax (8%)</span><span className="text-xs text-neutral-900">${tax.toFixed(2)}</span></div>
            <div className="flex justify-between pt-1"><span className="text-base font-bold text-neutral-900">Total</span><span className="text-base font-bold text-neutral-900">${total.toFixed(2)}</span></div>
          </div>

          {/* Payment */}
          <div className="mt-3 flex gap-2">
            {(["cash", "card", "wallet"] as const).map((m) => (
              <button key={m} onClick={() => setPayMethod(m)} className={`flex-1 rounded-lg py-1.5 text-center text-xs font-semibold capitalize ${payMethod === m ? "bg-purple-600 text-white" : "border border-neutral-100 text-neutral-600"}`}>{m}</button>
            ))}
          </div>

          <button onClick={handleCheckout} disabled={cart.length === 0 || processing} className={`mt-3 w-full rounded-xl py-3 text-sm font-bold text-white ${cart.length === 0 ? "bg-neutral-300" : "bg-emerald-500 hover:bg-emerald-600"}`}>
