...
export default async function BannedPage() {
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-neutral-50 px-4 py-12">
      <div className="w-full max-w-xl rounded-2xl border border-neutral-100 bg-white p-8 shadow-sm">
        <p className="text-xs font-semibold uppercase tracking-[0.28em] text-primary-600">
          {t("brand")}
        </p>
        <h1 className="mt-3 text-2xl font-semibold text-neutral-900">{t("title")}</h1>
        ...
        <div className="mt-6 flex flex-wrap gap-3">
          <Link
            href="/auth"
            className="rounded-full border border-neutral-100 px-5 py-2 text-sm font-semibold text-neutral-700 transition hover:border-primary-200 hover:text-primary-700"
          >
            {t("actions.back")}
          </Link>
          <form action={signOut}>
