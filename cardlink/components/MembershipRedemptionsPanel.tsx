  ...
  return (
    <section className="space-y-3">
      <div className="flex items-center justify-between gap-3">
        <h2 className="text-base font-semibold text-slate-900">{t("redeemHistory.title")}</h2>
        <button
          type="button"
      ...
      {message ? (
        <article className="rounded-2xl border border-violet-200 bg-violet-50 p-4 text-sm text-violet-700">
          {message}
        </article>
      ) : null}
          ...
          {rows.map((item) => (
            ...
            >
              <div className="flex flex-wrap items-start justify-between gap-2">
                <div>
                  <p className="text-sm font-semibold text-slate-900">{item.offerTitle}</p>
